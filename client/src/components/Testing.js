...
class Testing extends Component {
  ...
  loadReminders = () => {
    API.getReminders()
      .then(res => this.setState({ reminders: res.data, remain: "" }))
  ...
  getEvents = () => {
    ...
    API.getEvents("sanfrancisco").then(res => {
      ...
      .then(res => {
        ...
        for (var j = 4; j < 10; j++) {
          this.setState({
            weathers:
              "http://openweathermap.org/img/w/" +
              res.data.list[j].weather[0].icon +
              ".png"
          });
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <Button onClick={() => this.getHackJobs()}>Get Hack Jobs</Button>
        <div>{/* <p>Current time:</p> */}</div>
          ...
          <div>
            <MuiThemeProvider>
              <div>
                <form>
                  <Paper
                    style={{ width: "90%", leftMargin: "15px" }}
                    zDepth={1}
                  >
                    <div style={{ marginLeft: "10px" }}>
                      <TextField
